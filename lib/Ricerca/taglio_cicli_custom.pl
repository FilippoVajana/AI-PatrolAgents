...
%  verifica che il nodo non faccia già parte del cammino
taglia(N,L) :-
	in(N,L),
...
potatura(nc(passo(V,_T),Path,_),_,dl(L,L)) :-
	setof(E,(member(passo(E,_),Path)),ListaVisitati),
	member(V,ListaVisitati),!.
...
equals(passo(P,_T1),passo(P,_T2)).
