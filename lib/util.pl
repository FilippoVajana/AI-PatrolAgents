:- module(util, [
   win_format/2,
   %  win_format(+F,+L) equivale a format(TF,L) con ^ al posto di tilde

   win_format/3,
   %  win_format(+F,+L,+C) equivale a format(CF,L) con C al posto di tilde

   indent/1,
   %  indent(+K:number) scrive  K spazi

   write_indented/2
   %  write_indented(+K:number, +S:term) scrive K spazi	seguiti da S
	  ]).
