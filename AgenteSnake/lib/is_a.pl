...
check_goal(length(L,N), E1, E2) :-!,
	declared(type(list(_))),
	check_term(L, list(_),E1,E),
	check_term(N, number, E, E2).
check_goal(nth0(N,L,EL), E1, E2) :-!,
	declared(type(list(_))),
	check_term(L, list(Type),E1,E),
	check_term(N, number, E, EB),
	check_term(EL, Type, EB, E2).
check_goal(nth1(N,L,EL), E1, E2) :- !,
	declared(type(list(_))),
	check_term(L, list(Type),E1,E),
	check_term(N, number, E, EB),
	check_term(EL, Type, EB, E2).
...
check_goal(read(_), Env, Env) :- !.
check_goal(readln(_), Env, Env) :- !.
check_goal(read(_,_), Env, Env) :- !.
check_goal(readln(_,_), Env, Env) :- !.
...
check_goal(forall(Q,Body), Env1,Env2) :- !,
	check_body(Env1,EnvA,Q),
	check_body(EnvA, Env2, Body).
check_goal(call(Body), E1,E2) :-
	check_body(E1,E2,Body).
